...
const QRExporter = (() => {
  ...
  function downloadSVG(qrInstance) {
    qrInstance.download({ name: getFilename(), extension: 'svg' });
  }
  ...
  async function downloadPDF(qrInstance, title = '') {
    ...
    pdf.text('Generated by Free QR Generator', pageW / 2, pageH - 10, { align: 'center' });
  ...
  async function exportBatchItem(qrInstance, format = 'png') {
    if (format === 'svg') return getRawBlob(qrInstance, 'svg');
    let blob = await getRawBlob(qrInstance, 'png');
    if (typeof QRPro !== 'undefined') blob = await QRPro.applyWatermark(blob);
    return blob;
  }
